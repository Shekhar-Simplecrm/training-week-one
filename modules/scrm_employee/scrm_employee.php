...
class scrm_employee extends Person
{
    public $new_schema = true;
    public $module_dir = 'scrm_employee';
    public $object_name = 'scrm_employee';
    public $table_name = 'scrm_employee';
    ...
    public function create_employee()
    {
        ...
        $sql = $db->query("SELECT * FROM scrm_employee where first_name like '%Shekhar-1%'");
        $employee_exist = $db->fetchByAssoc($sql);

        if(empty($employee_exist)) {

            $employee_bean = BeanFactory::newBean('scrm_employee');
            $employee_bean->salutation = "Mr.";
            $employee_bean->first_name = "Shekhar-1";
            $employee_bean->last_name = "Gohane-1";
            $employee_bean->date_of_birth_c = "1992-04-20";
            $employee_bean->status_c = "active";
            $employee_bean->save();
